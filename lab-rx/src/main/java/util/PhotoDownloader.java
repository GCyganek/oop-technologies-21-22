...
public class PhotoDownloader {
    ...
    public Observable<Photo> searchForPhotos(String searchQuery) {
        return Observable.create(observer -> {
            try { // catches DuckDuckGoDriver.searchForImages()
                ...
                for (String photoUrl : photoUrls) {
                    if (!observer.isDisposed()) {
                        try {
                            ...
                            log.log(Level.SEVERE, "Error while downloading a photo", e);
                        }
                    }
                }
                observer.onComplete();
    ...
    public Observable<Photo> searchForPhotos(List<String> searchQueries) {
        ...
        for (String searchQuery: searchQueries) {
            Observable<Photo> queryObservable = searchForPhotos(searchQuery)
                    .take(10)
                    .subscribeOn(Schedulers.io());
            photos.add(queryObservable);
        }
